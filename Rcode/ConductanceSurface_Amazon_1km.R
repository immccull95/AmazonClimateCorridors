...
nacional_buff <- terra::vect("RAISG/vias2023-1/vias/buffered/vias_nacional_250m_buff.shp")
departamental_buff <- terra::vect("RAISG/vias2023-1/vias/buffered/vias_departamental_250m_buff.shp")
ferrea_buff <- terra::vect("RAISG/vias2023-1/vias/buffered/vias_ferrea_250m_buff.shp")
...
#writeVector(roads_union, filename="RAISG/vias2023-1/vias/buffered/roads_buffer_250m_union.shp", overwrite=T)
roads_union <- terra::vect("RAISG//vias2023-1/vias/buffered/roads_buffer_1000m_union.shp")

#plot(roads_union)
roads_union$road_field <- 222 #assign some obviously ridiculous number

# Convert roads to raster before integrating with LULC
lc_mosaic_1km <- terra::aggregate(lc_mosaic_500m, fact=2, fun="modal")
roads_raster <- terra::rasterize(roads_union, lc_mosaic_1km, field='road_field', background=NA)
#terra::writeRaster(roads_raster, filename='RAISG/vias2023-1/vias/buffered/roads_buffer_raster_1km.tif', overwrite=T)
#roads_raster <- terra::rast("RAISG/vias2023-1/vias/buffered/roads_buffer_raster_1km.tif")

# Mosaic land cover and roads
lc_road_mosaic <- terra::mosaic(lc_mosaic_1km, roads_raster, fun="max") #max: this is why we set road to very high value, so it will supersede any existing land cover value
plot(lc_road_mosaic)
...
lc_mosaic_1km_4326 <- terra::project(lc_mosaic_1km, "EPSG:4326") #just need the extent
canopy_amazon <- terra::crop(canopy, lc_mosaic_1km_4326, filename='tump/canopy_tump_1km.tif', overwrite=T)
ext(canopy_amazon)
...
# now reproject
canopy_amazon_a_29172 <- terra::project(canopy_amazon_a, "EPSG:29172", method="bilinear", res=c(1000,1000),
                                        filename='tump/GCH_amazon_29172_1km_a.tif', overwrite=T)
canopy_amazon_b_29172 <- terra::project(canopy_amazon_b, "EPSG:29172", method="bilinear", res=c(1000,1000),
                                        filename='tump/GCH_amazon_29172_1km_b.tif', overwrite=T)
canopy_amazon_c_29172 <- terra::project(canopy_amazon_c, "EPSG:29172", method="bilinear", res=c(1000,1000),
                                        filename='tump/GCH_amazon_29172_1km_c.tif', overwrite=T)
canopy_amazon_d_29172 <- terra::project(canopy_amazon_d, "EPSG:29172", method="bilinear", res=c(1000,1000),
                                        filename='tump/GCH_amazon_29172_1km_d.tif', overwrite=T)
...
canopy_amazon_mosaic <- terra::mosaic(canopy_amazon_sprc, fun="max", filename="GCH/GCH_amazon_29172_1km.tif", overwrite=T) #fun probably doesn't matter here
...
# canopy_amazon_29172 <- terra::project(canopy_amazon, "EPSG:29172", method="bilinear", res=c(1000,1000),
#                                       filename='GCH/GCH_amazon_29172_250m.tif', overwrite=T)
canopy_amazon_29172 <- terra::rast("GCH/GCH_amazon_29172_1km.tif")
...
#writeRaster(just_forest, "LCC/LCC_1km_forest.tif", overwrite=T)
...
writeRaster(conductance_final, filename='Conductance/amazon_conductance_1km.tif', overwrite=T)
