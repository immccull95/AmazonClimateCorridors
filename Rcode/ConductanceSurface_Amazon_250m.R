...
#roads_raster <- terra::rasterize(roads_union, lc_mosaic_250m, field='road_field', background=NA)
#terra::writeRaster(roads_raster, filename='RAISG/vias2023-1/vias/buffered/roads_buffer_raster_250m.tif', overwrite=T)
roads_raster <- terra::rast("RAISG/vias2023-1/vias/buffered/roads_buffer_raster_250m.tif")

# Mosaic land cover and roads
lc_road_mosaic <- terra::mosaic(lc_mosaic_250m, roads_raster, fun="max") #max: this is why we set road to very high value, so it will supersede any existing land cover value
plot(lc_road_mosaic)
...
lc_mosaic_250m_4326 <- terra::project(lc_mosaic_250m, "EPSG:4326") #just need the extent
canopy_amazon <- terra::crop(canopy, lc_mosaic_250m_4326, filename='tump/canopy_tump_250.tif', overwrite=T)
ext(canopy_amazon)
...
c <- ext(-60, -38.06275, -22.3605, -5)
c <- as.polygons(c, crs=crs(canopy_amazon))
d <- ext(-60, -38.06275, -5, 13.0005)
...
# now reproject
canopy_amazon_a_29172 <- terra::project(canopy_amazon_a, "EPSG:29172", method="bilinear", res=c(250,250),
                                        filename='tump/GCH_amazon_29172_250m_a.tif', overwrite=T)
canopy_amazon_b_29172 <- terra::project(canopy_amazon_b, "EPSG:29172", method="bilinear", res=c(250,250),
                                        filename='tump/GCH_amazon_29172_250m_b.tif', overwrite=T)
canopy_amazon_c_29172 <- terra::project(canopy_amazon_c, "EPSG:29172", method="bilinear", res=c(250,250),
                                        filename='tump/GCH_amazon_29172_250m_c.tif', overwrite=T)
canopy_amazon_d_29172 <- terra::project(canopy_amazon_d, "EPSG:29172", method="bilinear", res=c(250,250),
                                        filename='tump/GCH_amazon_29172_250m_d.tif', overwrite=T)
...
canopy_amazon_mosaic <- terra::mosaic(canopy_amazon_sprc, fun="max", filename="GCH/GCH_amazon_29172_250m.tif", overwrite=T) #fun probably doesn't matter here
...
# canopy_amazon_29172 <- terra::project(canopy_amazon, "EPSG:29172", method="bilinear", res=c(250,250),
#                                       filename='GCH/GCH_amazon_29172_250m.tif', overwrite=T)
canopy_amazon_29172 <- terra::rast("GCH/GCH_amazon_29172_250m.tif")
...
#writeRaster(just_forest, "LCC/LCC_250m_forest.tif", overwrite=T)
...
#writeRaster(conductance_final, filename='Conductance/amazon_conductance_250m.tif', overwrite=T)
